...
async function WhatsBATBAT () {
    var insult = await axios.get('https://https://gist.githubusercontent.com/valco72/1b8518727dcd1230b67df75e38040449/raw')
    const { shs1, shl2, lss3, dsl4 } = insult.data.inside
    await config.DATABASE.sync();
    var StrSes_Db = await WhatsBATBATDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
    ...
    conn.on ('credentials-updated', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await WhatsBATBATDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.WORKTYPE == 'public') {
            ...
            else { await conn.sendMessage(conn.user.jid, '*WhatsBATBAT Working as Public! 🐺*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as public. To change it, use* _.setvar WORK_TYPE:private_\n\n*Thanks for using WhatsBATBAT 💌*', MessageType.text);
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    await conn.sendMessage(
            ...
            else { await conn.sendMessage(conn.user.jid, '*WhatsBATBAT Working as Private! 🐺*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using WhatsBATBAT 💌*', MessageType.text);
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    await conn.sendMessage(
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (error.message.includes('429')) return;
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [WhatsBATBAT] --*' +
                                    '\n*WhatsBATBAT bir hata gerçekleşti!*'+
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [WhatsBATBAT] ⚕️*' +
                                        '\n========== ```Hata Okundu!``` ==========' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [WhatsBATBAT] --*' +
                                    '\n*WhatsBATBAT an error has occurred!*'+
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [WhatsBATBAT] ⚕️*' +
                                        '\n========== ```Error Resolved!``` ==========' +
